  ...
  namespace {
      ...
      if (kind == GamepadProfileKind::dualshock4) {
        switch (report_id) {
          ...
          case ps::dualshock4_bluetooth_calibration_report:
            if (request.bus == static_cast<std::uint32_t>(lvh::BusType::bluetooth)) {
    ...
    struct DeviceSession {
      ...
      void output(const std::uint8_t *data, std::size_t size, std::uint32_t report_id, const Message &profile) {
        ...
        if (profile.kind == static_cast<std::uint32_t>(lvh::GamepadProfileKind::switch_pro)) {
    ...
    IOHIDUserDeviceRef create_device(const Message &request, DeviceSession &session, dispatch_semaphore_t cancelled) {
      ...
      set_property(properties, CFSTR(kIOHIDTransportKey), request.bus == static_cast<std::uint32_t>(lvh::BusType::bluetooth) ? kIOHIDTransportBluetoothValue : kIOHIDTransportUSBValue);
