...
namespace lvh::detail::windows {
   ...
   * Bluetooth HID identity does, and SDL/Steam consume its 17-byte report
   * directly. This is a captured Microsoft 045E:0B13 descriptor; report ID 1
   * is controller input, report ID 2 is the consumer Home control, report ID 3
   * is rumble, and report ID 4 is battery state.
   *
   ...
   *
   * @param report Packed 17-byte Xbox GIP-compatible input report.
  ...
  inline std::vector<std::uint8_t> make_xbox_series_windows_input_report(
    ...
    const auto copy_button = [&buttons, &windows_report](std::uint16_t source_mask, std::size_t offset, std::uint8_t target_mask) {
      if ((buttons & source_mask) != 0U) {
        windows_report[offset] |= target_mask;
      }
    };

    copy_button(0x0001U, 14U, 0x01U);  // A
    copy_button(0x0002U, 14U, 0x02U);  // B
    copy_button(0x0004U, 14U, 0x08U);  // X
    copy_button(0x0008U, 14U, 0x10U);  // Y
    copy_button(0x0010U, 14U, 0x40U);  // LB
    copy_button(0x0020U, 14U, 0x80U);  // RB
    copy_button(0x0040U, 15U, 0x04U);  // Back
    copy_button(0x0080U, 15U, 0x08U);  // Start
    copy_button(0x0100U, 15U, 0x20U);  // L3
    copy_button(0x0200U, 15U, 0x40U);  // R3
    if (report[15] != 0U) {
      windows_report[15] |= 0x10U;  // Guide
    }
    copy_button(0x0800U, 16U, 0x01U);  // Share
